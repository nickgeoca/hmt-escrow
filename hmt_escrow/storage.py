...
def ipns_publish(hash_: str) -> str:
    ...
    """
    try:
        # docs for ipns publishing     ... https://ipfs.io/ipns/12D3KooWEqnTdgqHnkkwarSrJjeMP2ZJiADWLYADaNvUb6SQNyPF/docs/http_client_ref.html#ipfshttpclient.Client.name
        IPFS_CLIENT.name.publish('https://ipfs.io/ipfs/' + hash_)
    except Exception as e:
        LOG.warning("IPNS failed because of: {}".format(e))
        raise e

    # docs for getting the node id ... https://ipfs.io/ipns/12D3KooWEqnTdgqHnkkwarSrJjeMP2ZJiADWLYADaNvUb6SQNyPF/docs/http_client_ref.html#ipfshttpclient.Client.id
    return 'https://ipfs.io/ipns/' + IPFS_CLIENT.id()['ID']
